...
# Menu lateral
st.sidebar.image("https://via.placeholder.com/200x80?text=LTS+Construtora", width=200)
...
# Módulo Clientes
    ...
    with aba[0]:
        ...
        with st.form("form_cliente"):
            ...
            # Dados do cônjuge (aparece só se casado)
            ...
            if salvar:
                if not nome or not cpf or not telefone:
                    st.error("Preencha os campos obrigatórios: Nome, CPF e Telefone.")
                else:
                    try:
                        dados = {
                            "nome": nome,
    ...
    with aba[1]:
        ...
        busca = st.text_input("Digite o CPF ou nome")
        if busca:
            try:
                ...
                if resultado.data:
                    for c in resultado.data:
                        with st.expander(f"👤 {c['nome']} — CPF: {c['cpf']}"):
                            col1, col2 = st.columns(2)
                            with col1:
                                st.write(f"**Telefone:** {c.get('telefone','—')}")
                                st.write(f"**E-mail:** {c.get('email','—')}")
                                st.write(f"**Estado civil:** {c.get('estado_civil','—')}")
                                st.write(f"**Profissão:** {c.get('profissao','—')}")
                            with col2:
                                st.write(f"**Endereço:** {c.get('endereco','—')}")
                                st.write(f"**Cidade:** {c.get('cidade','—')}")
                                st.write(f"**Escolaridade:** {c.get('escolaridade','—')}")
                            if c.get('conjuge_nome'):
                                st.markdown("**Cônjuge:**")
                                st.write(f"Nome: {c.get('conjuge_nome','—')} | CPF: {c.get('conjuge_cpf','—')}")
                else:
    ...
    with aba[2]:
        ...
        try:
            todos = supabase.table("clientes").select("nome, cpf, telefone, cidade, estado_civil").execute()
            if todos.data:
